...
const CorrosionCompatibility: React.FC<CorrosionCompatibilityProps> = () => {
  const [data, setData] = useState<AlleimaCorrosionData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/materials/corrosion');
        if (!response.ok) {
  ...
  // 조건부 렌더링 전에 모든 Hook 호출 완료
  // React Hook 규칙: 조건부 렌더링 이후에 Hook을 호출할 수 없음
  ...
  const sortMaterialsByPriority = (materials: string[]): string[] => {
    return materials.sort((a, b) => {
      // Carbon steel을 Carbon Steel로 통일
      const normalizedA = a === 'Carbon steel' ? 'Carbon Steel' : a;
      const normalizedB = b === 'Carbon steel' ? 'Carbon Steel' : b;
      
      const aIndex = materialPriority.findIndex(priority =>
        normalizedA.includes(priority) || priority.includes(normalizedA)
      );
      const bIndex = materialPriority.findIndex(priority =>
        normalizedB.includes(priority) || priority.includes(normalizedB)
      );
  ...
  }, [data?.corrosion_data, selectedChemical, materialNameMap, sortMaterialsByPriority]);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 mb-3">
          {data && Object.entries(data.symbol_clarification).map(([symbol, description]) => (
  ...
  // 조건부 렌더링 - 모든 Hook 호출 후에 위치
  if (loading) {
    return <div className="flex justify-center items-center h-64">데이터 로딩 중...</div>;
  }

  if (error) {
    return <div className="flex justify-center items-center h-64 text-red-600">오류: {error}</div>;
  }

  if (!data) {
    return <div className="flex justify-center items-center h-64">데이터를 사용할 수 없습니다.</div>;
  }
